...
同一笔支出的发票、支付记录、订单截图、说明材料必须合并为同一行，不得按文件数量拆行；例如「kimi发票.pdf」+「kimi支付记录.jpg」应合并为「Kimi Vip 会员订阅服务」一行，「codex发票.pdf」+「codex支付凭证.jpg」应合并为「Codex/软件服务」一行。
不得输出「PDF发票1-8」「全部支出」「键盘」等无法对应真实支出项的泛化名称；若无法确定品名，应使用可识别的商户/服务名或文件名组合描述。`;
